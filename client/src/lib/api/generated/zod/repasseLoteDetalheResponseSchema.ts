/**
 * Generated by Kubb (https://kubb.dev/).
 * Do not edit manually.
 */

import { repasseLoteItemSchema } from "./repasseLoteItemSchema.ts";
import { z } from "zod/v4";

/**
 * @description Detalhes de um lote de repasses pagos
 */
export const repasseLoteDetalheResponseSchema = z
  .object({
    loteId: z.uuid(),
    colaboradorId: z.uuid(),
    dataRepasse: z.iso.datetime(),
    formaPagamento: z
      .enum(["PIX", "DINHEIRO", "CARTAO_CREDITO", "CARTAO_DEBITO", "BOLETO", "TRANSFERENCIA"])
      .describe("Forma de pagamento"),
    comprovanteUrl: z.string().nullish(),
    valorTotal: z.number(),
    quantidadeRepasses: z.int(),
    get repasses() {
      return z.array(repasseLoteItemSchema.describe("Repasse individual pertencente a um lote de pagamento"));
    },
  })
  .describe("Detalhes de um lote de repasses pagos");
