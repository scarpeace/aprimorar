/**
 * Generated by Kubb (https://kubb.dev/).
 * Do not edit manually.
 */

import type { EnderecoResponseDTO } from "./EnderecoResponseDTO.ts";

export const colaboradorDetailResponseDTOFuncaoEnum = {
  PROFESSOR: "PROFESSOR",
  ADMINISTRATIVO: "ADMINISTRATIVO",
  TERAPEUTA: "TERAPEUTA",
  MENTOR: "MENTOR",
} as const;

export type ColaboradorDetailResponseDTOFuncaoEnumKey =
  (typeof colaboradorDetailResponseDTOFuncaoEnum)[keyof typeof colaboradorDetailResponseDTOFuncaoEnum];

/**
 * @description Dados detalhados do colaborador
 */
export type ColaboradorDetailResponseDTO = {
  /**
   * @description Identificador unico do colaborador
   * @type string, uuid
   */
  id: string;
  /**
   * @description Nome completo do colaborador
   * @type string
   */
  nome: string;
  /**
   * @description Data de nascimento do colaborador
   * @type string, date
   */
  dataNascimento: string;
  /**
   * @description Chave PIX do colaborador
   * @type string
   */
  pix: string;
  /**
   * @description Telefone de contato do colaborador
   * @type string
   */
  telefone: string;
  /**
   * @description CPF do colaborador
   * @type string
   */
  cpf: string;
  /**
   * @description Email do colaborador
   * @type string
   */
  email: string;
  /**
   * @description Função/Papel do funcionário
   * @type string
   */
  funcao: ColaboradorDetailResponseDTOFuncaoEnumKey;
  /**
   * @description Dados do endereco retornados pela API
   * @type object
   */
  endereco: EnderecoResponseDTO;
  /**
   * @description Indica se o colaborador está ativo
   * @type boolean | undefined
   */
  active?: boolean;
  /**
   * @description Data de criacao do colaborador
   * @type string, date-time
   */
  createdAt: string;
  /**
   * @description Data da ultima atualizacao do colaborador
   * @type string, date-time
   */
  updatedAt?: string | null;
};
