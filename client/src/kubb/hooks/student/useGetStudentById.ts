...
export const getStudentByIdQueryKey = (
  studentId: GetStudentByIdPathParams["studentId"],
) =>
  [
    { url: "/v1/students/:studentId", params: { studentId: studentId } },
  ] as const;
...
export async function getStudentById(
  ...
  const res = await request<
    GetStudentByIdQueryResponse,
    ResponseErrorConfig<GetStudentById400>,
    unknown
  >({
    method: "GET",
...
>(
  ...
  const queryKey =
    resolvedOptions?.queryKey ?? getStudentByIdQueryKey(studentId);
  ...
  const query = useQuery(
    ...
    queryClient,
  ) as UseQueryResult<TData, ResponseErrorConfig<GetStudentById400>> & {
    queryKey: TQueryKey;
  };
