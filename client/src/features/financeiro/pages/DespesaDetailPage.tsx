...
export function DespesaDetailPage() {
  const { id } = useParams<{ id: string }>();
  const despesaId = id ?? "";
  const navigate = useNavigate();
  const [isEditOpen, setIsEditOpen] = useState(false);
  const [isDeleteOpen, setIsDeleteOpen] = useState(false);
  const despesaQuery = useGetDespesaById(despesaId);
  const { deleteExpense } = useDespesaMutations();
    ...
    return (
      <PageLayout {...headerProps}>
        <ErrorCard
          ...
          error={despesaQuery.error}
        />
      </PageLayout>
    );
  }

  if (despesaQuery.isPending || !despesaQuery.data) {
    return (
      <PageLayout {...headerProps}>
  ...
  const despesa = despesaQuery.data;
  const isPaid = !!despesa.paymentDate;
  const categoryLabel = despesa.category
    ? DESPESA_CATEGORY_LABEL[despesa.category] ?? despesa.category
    : "Sem categoria";
    ...
    <PageLayout {...headerProps}>
      <section className="rounded-2xl border border-base-300 bg-base-100 p-5 shadow-sm animate-[fade-up_300ms_ease-out_both]">
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          <DetailItem
            label="Valor"
            value={brl.format(despesa.amount ?? 0)}
            className="font-mono text-error"
          />
          <DetailItem
            label="Data"
            value={despesa.date ? formatDateShortYear(despesa.date) : "-"}
          />
          <DetailItem
            label="Pagamento"
            value={
              despesa.paymentDate
                ? formatDateShortYear(despesa.paymentDate)
                : "Pendente"
            }
            className={isPaid ? "text-success" : "text-warning"}
          />
          <DetailItem label="Categoria" value={categoryLabel} />
          <DetailItem label="Identificador" value={despesa.id ?? "-"} />
        </div>
      </section>
