...
import { useStudentsByParentQuery } from "@/features/parents/hooks/use-parents"
...
export function StudentsTable({ variant = "studentsPage", ownerId, searchTerm = "" }: Readonly<StudentsTableProps>) {
  ...
  const studentsPageResults = useStudentsQuery(currentPage, pageSize, searchTerm, {
  ...
  const isLoading = variant === "studentsPage" ? studentsPageResults.isLoading : parentStudentsResults.isLoading
  const error = variant === "studentsPage" ? studentsPageResults.error : parentStudentsResults.error
  const data = variant === "studentsPage" ? studentsPageResults.data : parentStudentsResults.data

  const studentList: StudentResponse[] = Array.isArray(data) ? data : data?.content ?? []
  ...
  if (totalElements === 0 && !searchTerm) {
    return <EmptyCard description="Cadastre um aluno para ele aparecer por aqui." title={"Nenhum aluno encontrado."} />
  }

  if (error) {
    return <ErrorCard description={error.message || "Ocorreu um erro."} title="Erro ao carregar alunos" />
  }
        ...
        <table className="table table-zebra w-full">
          <thead className="bg-base-200/90">
            <tr>
              ...
              {variant === "studentsPage" && <th className="app-th hidden lg:table-cell">Email</th>}
              <th className="app-th">Escola</th>
              {variant === "embeddedParent" && <th className="app-th-center">Idade</th>}
              {variant === "embeddedParent" && <th className="app-th">Contato</th>}
              {variant === "studentsPage" && <th className="app-th">Responsável</th>}
              {variant === "studentsPage" && <th className="app-th-center">Status</th>}
          ...
          <tbody>
            {studentList.map((student) => (
              <tr className="transition-colors hover:bg-base-200/70" key={student.id}>
                ...
                {variant === "studentsPage" && <td className="hidden lg:table-cell">{student.email}</td>}
                <td>{student.school}</td>
                {variant === "embeddedParent" && <td className="text-center">{student.age}</td>}
