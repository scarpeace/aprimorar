...
import {
  DESPESA_CATEGORY_LABEL,
  DESPESA_CATEGORY_ORDER,
} from "../lib/despesa-category-labels";

const DESPESA_CATEGORY_COLORS: Record<string, string> = {
  CONTAS: "#475569",
...
export function DespesasPieChart({
  ...
  isPending,
  error,
}: Readonly<DespesasPieChartProps>) {
  const totals = (despesas ?? []).reduce<Record<string, number>>(
    (accumulator, despesa) => {
      const category = despesa.category ?? "SEM_CATEGORIA";
      accumulator[category] =
        (accumulator[category] ?? 0) + (despesa.amount ?? 0);
      return accumulator;
    },
    {},
  );
  ...
  const orderedCategories = [
    ...DESPESA_CATEGORY_ORDER.filter((category) => (totals[category] ?? 0) > 0),
    ...Object.keys(totals).filter(
      (category) => !DESPESA_CATEGORY_ORDER.includes(category),
    ),
  ];

  const chartData: ChartDatum[] = orderedCategories
    .map((category, index) => ({
      name: DESPESA_CATEGORY_LABEL[category] ?? "Sem categoria",
      value: totals[category] ?? 0,
      fill:
        DESPESA_CATEGORY_COLORS[category] ??
