...
import { DespesaCategoryBadge } from "./DespesaCategoryBadge";
import { ToggleDespesaPaymentButton } from "./ToggleDespesaPaymentButton";
import { useNavigate } from "react-router-dom";


type DespesasTableProps = {
  despesas?: PageDTODespesaResponseDTO;
  isPending: boolean;
  error: unknown;
};
    ...
    <div className="max-h-120 overflow-y-auto overflow-x-auto rounded-2xl border border-base-300 bg-base-100 shadow-lg">
      <table className="table table-zebra bg-base-100 animate-[fade-up_280ms_ease-out_both]">
        ...
        <tbody className="whitespace-nowrap">
          {despesas?.content.map((despesa) => (
            <tr
              key={despesa.id ?? `${despesa.date}-${despesa.description}`}
            >
              <td className="font-medium text-base-content/70">
                {despesa.date ? formatDateShortYear(despesa.date) : "-"}
              </td>
              <td className="font-bold text-base-content">
                {despesa.description ?? "Despesa sem descricao"}
              </td>
              <td>
                <DespesaCategoryBadge category={despesa.category} />
              </td>
              <td
                className={`text-right font-mono font-semibold ${
                  despesa.paymentDate ? "text-success" : "text-base-400"
                }`}
                title={despesa.paymentDate ? "Paga" : "Pendente"}
              >
                {brl.format(despesa.amount ?? 0)}
              </td>
              <td>
                <div className="flex justify-end gap-2">

                    <ToggleDespesaPaymentButton
                      iconOnly
                      isPaid={!!despesa.paymentDate}
                    ...
                    <Button
                      ...
                      onClick={() => navigate(`/finance/despesas/${despesa.id}`)}
                    >
