...
export const atendimentoFormSchema = z.object({
  ...
  content: z.enum(atendimentoContentOptions, {
    error: "O tipo do atendimentoo é obrigatório",
  }),
  startDate: z.string().min(1, { message: "A data de início do atendimentoo é obrigatória" }),
  duration: z.number({ error: "A duração do atendimentoo é obrigatória" }).min(0.5, { message: "A duração mínima é de 30 minutos (0.5h)" }),
  price: z
    .number({ error: "O preço do atendimentoo é obrigatório" })
    .min(0, { message: "O preço deve ser maior ou igual a zero" }),
  payment: z
    .number({ error: "O pagamento do atendimentoo é obrigatório" })
