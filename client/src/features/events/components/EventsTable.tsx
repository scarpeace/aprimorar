...
export function EventsTable({
  ...
  const employeeResults = useEventsByEmployeeQuery(
    ownerId!,
    currentPage,
    pageSize,
    {
  ...
  const studentResults = useEventsByStudentQuery(
    ownerId!,
    currentPage,
    pageSize,
    {
  ...
  if (events?.page.totalElements === 0) {
    return (
      <EmptyCard
        description="Cadastre um evento para ele aparecer por aqui."
  ...
  if (error) {
    return (
      <ErrorCard description={error.message} title="Erro ao carregar eventos" />
    );
  }
      ...
      <table className="table table-zebra w-full">
        <thead className="bg-base-200/90">
          <tr>
            ...
            {showPrice ? (
              <th className="app-th hidden lg:table-cell">Valor</th>
            ) : null}
            {showPayment ? (
              <th className="app-th hidden lg:table-cell">Pagamento</th>
            ) : null}
