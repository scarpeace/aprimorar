...
export const studentsApi = {
  list: (page = 0, size = 20, sortBy = "name", includeArchived?: boolean): Promise<AxiosResponse<PageResponse<StudentResponse>>> => {
    ...
    return api.get(`/v1/students?page=${page}&size=${size}&sort=${sortBy}`)
  },
  ...
  update: (id: string, data: CreateStudentInput): Promise<AxiosResponse<StudentResponse>> => api.put(`/v1/students/${id}`, data),
...
export const employeesApi = {
  ...
  includeArchived = false
    ...
    void includeArchived
    return api.get(`/v1/employees?page=${page}&size=${size}&sort=${sortBy}`)
  },
...
export const eventsApi = {
  list: (page = 0, size = 20, sortBy = "startDateTime"): Promise<AxiosResponse<PageResponse<EventResponse>>> =>
    api.get(`/v1/events?page=${page}&size=${size}&sort=${sortBy}`),
  listByStudent: (
    studentId: string,
    page = 0,
    size = 20,
    ...
    api.get(`/v1/events/student/${studentId}?page=${page}&size=${size}&sort=${sortBy}`),
  listByEmployee: (
    employeeId: string,
    page = 0,
    size = 20,
    ...
    api.get(`/v1/events/employee/${employeeId}?page=${page}&size=${size}&sort=${sortBy}`),
  getById: (id: string): Promise<AxiosResponse<EventResponse>> => api.get(`/v1/events/${id}`),
  create: (data: CreateEventInput): Promise<AxiosResponse<EventResponse>> => api.post("/v1/events", data),
  update: (id: string, data: CreateEventInput): Promise<AxiosResponse<EventResponse>> => api.put(`/v1/events/${id}`, data),
...
export const parentsApi = {
  ...
  includeArchived = false
    ...
    void includeArchived
    return api.get(`/v1/parents?page=${page}&size=${size}&sort=${sortBy}`)
  },
  getById: (id: string): Promise<AxiosResponse<ParentResponse>> => api.get(`/v1/parents/${id}`),
}
