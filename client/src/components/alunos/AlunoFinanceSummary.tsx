...
import { useGetResumoFinanceiroAluno } from "@/lib/api/generated/hooks/atendimento/useGetResumoFinanceiroAluno";
import { ErrorCard } from "@/components/ui/ErrorCard";
import { getFriendlyErrorMessage } from "@/lib/api/client";
import { brl } from "@/lib/utils/formatter";

type AlunoFinanceSummaryProps = {
  alunoId: string;
  anoMes: string;
};
...
export function AlunoFinanceSummary({ alunoId, anoMes }: Readonly<AlunoFinanceSummaryProps>) {
  const resumo = useGetResumoFinanceiroAluno(
    alunoId,
    { anoMes },
    {
      query: {
    ...
    <section className="rounded-2xl border border-base-300 bg-base-100 p-6 shadow-sm">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          <p className="mt-2 text-sm text-base-content/65">Acompanhe o pagamento recebido e o pagamento pendente do período selecionado.</p>
        </div>

        <div className="grid gap-3 sm:grid-cols-2">
          <SummaryCard label="Pagamento recebido" value={resumo.data?.totalPago} tone="success" />
          <SummaryCard label="Pagamento pendente" value={resumo.data?.totalPendente} tone="warning" />
        </div>
      </div>
    </section>
  );
