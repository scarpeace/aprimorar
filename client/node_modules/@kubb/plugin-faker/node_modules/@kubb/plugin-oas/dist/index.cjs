...
const require_requestBody = require("./requestBody-DWzEIpum.cjs");
...
function findSchemaKeyword(siblings, keyword) {
	return require_requestBody.SchemaGenerator.find(siblings, require_SchemaMapper.schemaKeywords[keyword]);
}
//#endregion
...
var OperationGenerator = class {
	...
	getSchemas(operation, { resolveName = (name) => name } = {}) {
		...
		const operationName = require_getFooter.pascalCase(operationId);
		...
		const statusCodes = operation.getResponseStatusCodes().map((statusCode) => {
			...
			return {
				name: this.context.UNSTABLE_NAMING ? resolveName(require_getFooter.pascalCase(`${operationId} status ${name}`)) : resolveName(require_getFooter.pascalCase(`${operationId} ${name}`)),
		...
		const request = require_requestBody.withRequiredRequestBodySchema(requestSchema ? {
			name: this.context.UNSTABLE_NAMING ? resolveName(require_getFooter.pascalCase(`${operationId} RequestData`)) : resolveName(require_getFooter.pascalCase(`${operationId} ${operation.method === "get" ? "queryRequest" : "mutationRequest"}`)),
		...
		return {
			pathParams: pathParamsSchema ? {
				name: resolveName(require_getFooter.pascalCase(`${operationId} PathParams`)),
				operation,
				operationName,
			...
			queryParams: queryParamsSchema ? {
				name: resolveName(require_getFooter.pascalCase(`${operationId} QueryParams`)),
				operation,
				operationName,
			...
			headerParams: headerParamsSchema ? {
				name: resolveName(require_getFooter.pascalCase(`${operationId} HeaderParams`)),
				operation,
				operationName,
			...
			request,
			response: {
				name: this.context.UNSTABLE_NAMING ? resolveName(require_getFooter.pascalCase(`${operationId} ResponseData`)) : resolveName(require_getFooter.pascalCase(`${operationId} ${operation.method === "get" ? "queryResponse" : "mutationResponse"}`)),
				operation,
				operationName,
		...
		inject() {
			...
			return {
				...
				async getBaseURL() {
					...
					return (0, _kubb_oas.resolveServerUrl)(server, serverVariables);
				}
			};
		},
			...
			*/
			...
			if (group && (options?.group?.path || options?.group?.tag)) {
				const groupName = group?.name ? group.name : (ctx) => {
					...
					return `${require_getFooter.camelCase(ctx.group)}Controller`;
				};
