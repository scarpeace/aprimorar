...
var SchemaGenerator = class SchemaGenerator {
	...
	*/
	parse(props) {
		...
		const result = (0, remeda.uniqueWith)(options.transformers?.schema?.(props, defaultSchemas) || defaultSchemas || [], remeda.isDeepEqual);
	...
	*/
	#parseProperties(name, schemaObject, rootName) {
		...
		const propertiesSchemas = Object.keys(properties).map((propertyName) => {
			...
			const nullable = (0, _kubb_oas.isNullable)(propertySchema);
	...
	*/
	#getRefAlias(schemaObject, name) {
		...
		if ($ref.startsWith("#") && !$ref.startsWith("#/components/")) try {
			...
			if (inlineSchema && !(0, _kubb_oas.isReference)(inlineSchema)) return this.parse({
				schema: inlineSchema,
				name,
				parentName: null,
				rootName: null
	...
	#addDiscriminatorToSchema({ schema, schemaObject, discriminator }) {
		...
		return {
			...
			args: Object.entries(discriminator.mapping || {}).map(([key, value]) => {
				let arg;
				if (value.startsWith(_kubb_oas.KUBB_INLINE_REF_PREFIX)) {
					const index = Number.parseInt(value.replace(_kubb_oas.KUBB_INLINE_REF_PREFIX, ""), 10);
					if (!Number.isNaN(index) && index >= 0 && index < schema.args.length) arg = schema.args[index];
				} else arg = schema.args.find((item) => require_SchemaMapper.isKeyword(item, require_SchemaMapper.schemaKeywords.ref) && item.args.$ref === value);
				if (!arg) return;
				return {
					keyword: require_SchemaMapper.schemaKeywords.and,
					args: [arg, {
	...
	*/
	#parseSchemaObject({ schema: _schemaObject, name, parentName, rootName }) {
		...
		if (max !== void 0) if (exclusiveMaximum) baseItems.unshift({
			keyword: require_SchemaMapper.schemaKeywords.exclusiveMaximum,
			args: max
		});
		...
		if (min !== void 0) if (exclusiveMinimum) baseItems.unshift({
			keyword: require_SchemaMapper.schemaKeywords.exclusiveMinimum,
			args: min
		});
		else baseItems.unshift({
		...
		if (schemaObject.type && Array.isArray(schemaObject.type)) {
			...
			if (items.length > 1) return [...[{
				...
				args: items.map((item) => this.parse({
					...
					parentName,
					rootName
				})[0]).filter((x) => Boolean(x)).map((item) => require_SchemaMapper.isKeyword(item, require_SchemaMapper.schemaKeywords.object) ? {
					...item,
					args: {
						...item.args,
						strict: true
		...
		if ((0, _kubb_oas.isReference)(schemaObject)) return [
			...
			...baseItems.filter((item) => item.keyword === require_SchemaMapper.schemaKeywords.default),
		...
		if (schemaObject.allOf) {
			...
			if (schemaWithoutAllOf.required?.length) {
				...
				for (const item of allOfItems) {
					const resolved = (0, _kubb_oas.isReference)(item) ? this.context.oas.get(item.$ref) : item;
		...
		*/
		if (schemaObject.type === "string" && schemaObject.contentMediaType === "application/octet-stream") {
			baseItems.unshift({ keyword: require_SchemaMapper.schemaKeywords.blob });
			return baseItems;
		}
		if (schemaObject.format) {
			if (schemaObject.type === "integer" && schemaObject.format === "int64") {
				baseItems.unshift({ keyword: options.integerType === "bigint" ? require_SchemaMapper.schemaKeywords.bigint : require_SchemaMapper.schemaKeywords.integer });
				return baseItems;
			}
...
//# sourceMappingURL=requestBody-DWzEIpum.cjs.map