...
var URLPath = class {
	...
	*/
	...
	#transformParam(raw) {
		const param = isValidVarName(raw) ? raw : require_getFooter.camelCase(raw);
		return this.#options.casing === "camelcase" ? require_getFooter.camelCase(param) : param;
	}
...
function getImports(tree) {
	const refs = require_requestBody.SchemaGenerator.deepSearch(tree, require_SchemaMapper.schemaKeywords.ref);
	if (!refs) return [];
...
function getASTParams(operationSchema, { typed = false, casing, override } = {}) {
	...
	return Object.entries(operationSchema.schema.properties).map(([name]) => {
		const accessName = casing === "camelcase" ? require_getFooter.camelCase(name) : name;
		const data = {
			name,
			enabled: !!name,
...
function getPathParams(operationSchema, options = {}) {
	return getASTParams(operationSchema, options).reduce((acc, curr) => {
		if (curr.name && curr.enabled) {
			let name = curr.name;
			if (options.casing === "camelcase") name = require_getFooter.camelCase(name);
			else if (!isValidVarName(name)) name = require_getFooter.camelCase(name);
			acc[name] = {
...
function getParamsMapping(operationSchema, options = {}) {
	if (!operationSchema?.schema.properties) return;
	const allEntries = [];
	let hasTransformation = false;
	Object.entries(operationSchema.schema.properties).forEach(([originalName]) => {
		let transformedName = originalName;
		if (options.casing === "camelcase") transformedName = require_getFooter.camelCase(originalName);
		else if (!isValidVarName(originalName)) transformedName = require_getFooter.camelCase(originalName);
...
function applyParamsCasing(schema, casing) {
	...
	Object.entries(schema.properties).forEach(([originalName, propertySchema]) => {
		let transformedName = originalName;
		if (casing === "camelcase") transformedName = require_getFooter.camelCase(originalName);
		else if (!isValidVarName(originalName)) transformedName = require_getFooter.camelCase(originalName);
...
exports.getSchemaFactory = require_requestBody.getSchemaFactory;
...
exports.isRequestBodyRequired = require_requestBody.isRequestBodyRequired;
exports.withRequiredRequestBodySchema = require_requestBody.withRequiredRequestBodySchema;
