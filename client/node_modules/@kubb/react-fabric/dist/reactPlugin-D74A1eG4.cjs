...
const Renderer = (0, import_react_reconciler.default)({
	...
	createInstance(originalType, newProps, _root) {
		...
		for (const [key, value] of Object.entries(newProps)) {
			...
			if (value !== void 0) setAttribute(node, key, value);
		}
		return node;
	},
	...
	commitUpdate(node, _payload, _type, _oldProps, newProps) {
		...
		if (props) {
			for (const [key, value] of Object.entries(props)) if (value !== void 0) setAttribute(node, key, value);
		}
	},
...
var Runtime = class {
	...
	constructor(options) {
		...
		this.unsubscribeExit = (0, _kubb_fabric_core.onProcessExit)((code) => {
			this.unmount(code);
		});
	}
	get fileManager() {
	...
	onRender = () => {
		this.#renderPromise = this.#renderPromise.catch(() => {}).then(async () => {
			...
			if (this.#options?.stdout && node_process.default.env.NODE_ENV !== "test") {
	...
	async render(node) {
		const treeNode = this.#options.treeNode || new _kubb_fabric_core.TreeNode({
			type: "Root",
			props: {}
		});
		const props = {
