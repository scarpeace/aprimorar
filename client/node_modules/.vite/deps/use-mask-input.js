...
// node_modules/use-mask-input/dist/chunk-XSTQDKDU.js
...
function useMaskInput(props) {
  const { mask: mask2, register, options } = props;
  const ref = (0, import_react.useRef)(null);
  const maskRef = (0, import_react.useRef)(mask2);
  const optionsRef = (0, import_react.useRef)(options);
  const unmaskedValue = (0, import_react.useCallback)(() => getUnmaskedValue(ref.current), []);
  const refCallback = (0, import_react.useCallback)((input) => {
    if (!input) {
      ref.current = null;
      return;
    }
    ...
    return (fieldName, mask2, options) => {
      ...
      if (!entry) {
        ...
        const applyMaskToRef = (_ref) => {
          ...
          if (_ref) applyMaskToElement(_ref, mask2, options);
          return _ref;
        };
...
function withTanStackFormMask(inputProps, mask2, options) {
  ...
  if (!ref) {
    ...
    const result2 = {
      ...
      ref: ((input) => {
        ...
        if (input) applyMaskToElement(input, mask2, options);
      })
    };
  ...
  const cacheKey = makeMaskCacheKey(inputProps.name ?? "", mask2);
...
function useTanStackFormMask() {
  return (0, import_react.useMemo)(
    () => (mask2, inputProps, options) => withTanStackFormMask(inputProps, mask2, options),
    []
  );
}
var refCache2 = /* @__PURE__ */ new WeakMap();
function withHookFormMask(register, mask2, options) {
  const { ref } = register;
  if (!ref) {
    const result2 = {
      ...register,
  ...
  const cacheKey = makeMaskCacheKey(register.name, mask2);
  if (!maskCache?.has(cacheKey)) {
    const maskedRef2 = ((input) => {
      maskedRef2.currentElement = input;
      if (input) applyMaskToElement(input, mask2, options);
