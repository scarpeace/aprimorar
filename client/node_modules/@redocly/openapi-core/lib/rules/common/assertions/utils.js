...
export function getProblemsMessage(problems) {
    return problems.length === 1
        ? (problems[0].message ?? '')
        : problems.map((problem) => `\n- ${problem.message ?? ''}`).join('');
}
export function getFilenameFromPath(absoluteRef) {
...
export function interpolateMessagePlaceholders(message, placeholders) {
    let result = message;
