...
export const NoUnusedComponents = () => {
    ...
    const securitySchemeLocations = new Map();
    const usedSecuritySchemeComponents = new Set();
    ...
    return {
        ...
        NamedSchemas: {
            Schema(schema, { location, key, resolve }) {
                const referencesDiscriminator = schema.allOf?.some((ref) => isRef(ref) && resolve(ref)?.node?.discriminator);
