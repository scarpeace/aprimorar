export function validateMimeTypeOAS2({ type, value, ctx, allowedValues, reference, }) {
...
export function validateMimeTypeOAS3({ type, value, ctx, allowedValues, reference, }) {
    const { report, location } = ctx;
    const ruleType = type === 'consumes' ? 'request' : 'response';
    if (!allowedValues)
        throw new Error(`Parameter "allowedValues" is not provided for "${ruleType}-mime-type" rule`);
