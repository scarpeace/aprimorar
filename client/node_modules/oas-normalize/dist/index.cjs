...
var _chunkOCV23LAOcjs = require('./chunk-OCV23LAO.cjs');

// src/index.ts
var _fs = require('fs'); var _fs2 = _interopRequireDefault(_fs);
var _openapiparser = require('@readme/openapi-parser');
var _urls = require('@readme/openapi-parser/lib/urls');
...
var OASNormalize = class _OASNormalize {
  ...
  constructor(file, opts) {
    ...
    this.type = _chunkOCV23LAOcjs.getType.call(void 0, this.file);
    this.cache = {
      load: false,
      bundle: false,
      deref: false
  ...
  async load() {
    ...
    const resolve = (obj) => {
      const ret = _chunkOCV23LAOcjs.stringToJSON.call(void 0, obj);
      if (!ret) {
    ...
    switch (this.type) {
      ...
      case "url": {
        const { url, options } = _chunkOCV23LAOcjs.prepareURL.call(void 0, this.file);
        if (_urls.isUnsafeURL.call(void 0, url)) {
  ...
  async bundle() {
    ...
    return this.load().then((schema) => {
      if (_chunkOCV23LAOcjs.isPostman.call(void 0, schema)) {
  ...
  async convert() {
    ...
    return this.load().then(async (schema) => {
      return _chunkOCV23LAOcjs.isPostman.call(void 0, schema) ? _OASNormalize.convertPostmanToOpenAPI(schema) : schema;
    }).then(async (schema) => {
      if (!_chunkOCV23LAOcjs.isSwagger.call(void 0, schema) && !_chunkOCV23LAOcjs.isOpenAPI.call(void 0, schema)) {
        throw new Error("The supplied API definition is unsupported.");
      } else if (_chunkOCV23LAOcjs.isOpenAPI.call(void 0, schema)) {
        return schema;
      }
  ...
  async validate(opts = {}) {
    ...
    return this.load().then(async (schema) => {
      ...
      } else if (_chunkOCV23LAOcjs.isSwagger.call(void 0, schema)) {
        const baseVersion = parseInt(schema.swagger, 10);
        if (baseVersion === 1) {
          if (shouldThrowIfInvalid) {
  ...
  async version() {
    return this.load().then((schema) => {
      switch (_chunkOCV23LAOcjs.getAPIDefinitionType.call(void 0, schema)) {
        case "openapi":
          return {
