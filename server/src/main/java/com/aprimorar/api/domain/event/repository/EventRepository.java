...
public interface EventRepository extends JpaRepository<Event, UUID>, JpaSpecificationExecutor<Event> {
    ...
    @Query("SELECT COALESCE(SUM(e.price), 0) FROM Event e WHERE e.student.id = :studentId AND e.studentChargeDate IS NOT NULL")
    BigDecimal sumChargedByStudentId(@Param("studentId") UUID studentId);

    @Query("SELECT COALESCE(SUM(e.price), 0) FROM Event e WHERE e.student.id = :studentId AND e.studentChargeDate IS NULL")
