...
public interface EventRepository extends JpaRepository<Event, UUID>, JpaSpecificationExecutor<Event> {
    ...
    @Modifying
    @Query("UPDATE Event e SET e.studentId = :ghostId WHERE e.studentId = :studentId")
    void reassignStudentEventsToGhost(@Param("studentId") UUID studentId, @Param("ghostId") UUID ghostId);

    @Modifying
    @Query("UPDATE Event e SET e.employeeId = :ghostId WHERE e.employeeId = :employeeId")
    ...
    @Query("SELECT COALESCE(SUM(e.payment), 0) FROM Event e WHERE e.employeeId = :employeeId AND e.startDate BETWEEN :startDate AND :endDate AND e.employeePaymentDate IS NOT NULL")
    ...
    @Query("SELECT COALESCE(SUM(e.payment), 0) FROM Event e WHERE e.employeeId = :employeeId AND e.startDate BETWEEN :startDate AND :endDate AND e.employeePaymentDate IS NULL")
    ...
    @Query("SELECT COALESCE(SUM(e.payment), 0) FROM Event e WHERE e.employeeId = :employeeId AND e.employeePaymentDate IS NOT NULL")
    BigDecimal sumPaidByEmployeeId(@Param("employeeId") UUID employeeId);

    @Query("SELECT COALESCE(SUM(e.payment), 0) FROM Event e WHERE e.employeeId = :employeeId AND e.employeePaymentDate IS NULL")
    ...
    @Query("SELECT COALESCE(SUM(e.price), 0) FROM Event e WHERE e.studentId = :studentId AND e.startDate BETWEEN :startDate AND :endDate AND e.studentChargeDate IS NOT NULL")
    ...
    @Query("SELECT COALESCE(SUM(e.price), 0) FROM Event e WHERE e.studentId = :studentId AND e.startDate BETWEEN :startDate AND :endDate AND e.studentChargeDate IS NULL")
    ...
    @Query("SELECT COALESCE(SUM(e.price), 0) FROM Event e WHERE e.studentId = :studentId AND e.studentChargeDate IS NOT NULL")
    BigDecimal sumChargedByStudentId(@Param("studentId") UUID studentId);

    @Query("SELECT COALESCE(SUM(e.price), 0) FROM Event e WHERE e.studentId = :studentId AND e.studentChargeDate IS NULL")
    ...
    @Query(
        ...
        where e.startDate >= :startDate
          and e.startDate < :endDate
          and e.studentId <> :excludedStudentId
        """
    )
