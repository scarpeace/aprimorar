...
public class TransacaoMutationController {
    ...
    public ResponseEntity<TransacaoResponseDTO> createTransacao(@RequestBody @Valid TransacaoRequestDTO dto) {
        var response = transacaoMutationService.criarSaidaDespesa(dto);
        return ResponseEntity.created(URI.create("/v1/transacoes/" + response.id())).body(response);
    }

    @PostMapping("/{transacaoId}/efetivar")
