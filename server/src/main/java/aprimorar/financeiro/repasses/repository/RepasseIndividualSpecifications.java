...
public final class RepasseIndividualSpecifications {
    ...
    private static Specification<RepasseIndividual> statusIgual(StatusRepasseIndividual status) {
        return (root, query, cb) -> status == null ? null : cb.equal(root.get("status"), status);
    }

    private static Specification<RepasseIndividual> formaPagamentoIgual(FormaPagamentoEnum formaPagamento) {
        return (root, query, cb) -> formaPagamento == null
            ? null
            : cb.equal(root.get("formaPagamento"), formaPagamento);
    }

    private static Specification<RepasseIndividual> dataRepasseMaiorOuIgual(LocalDate data) {
        return (root, query, cb) -> data == null
            ? null
            : cb.greaterThanOrEqualTo(root.<LocalDateTime>get("dataRepasse"), data.atStartOfDay());
    }

    private static Specification<RepasseIndividual> dataRepasseMenorOuIgual(LocalDate data) {
        return (root, query, cb) -> data == null
            ? null
