package aprimorar.financeiro.cobrancas.domain;
...
public class CobrancaIndividual {
    ...
    protected CobrancaIndividual() {}

    public CobrancaIndividual(Long atendimentoId, UUID alunoId, BigDecimal valor) {
        validarValor(valor);
    ...
    public void registrarPagamento(UUID loteId, FormaPagamentoEnum formaPagamento, String comprovanteUrl) {
        if (status == StatusCobrancaIndividual.PAGO) {
            throw new CobrancaIndividualDadosInvalidosException("Cobrança já está paga");
        }
        if (status == StatusCobrancaIndividual.CANCELADO) {
    ...
    public void cancelar() {
        if (status != StatusCobrancaIndividual.PENDENTE) {
            throw new CobrancaIndividualDadosInvalidosException(
                "A cobrança precisa estar pendente para ser cancelada"
            );
        }

        this.status = StatusCobrancaIndividual.CANCELADO;
    }
